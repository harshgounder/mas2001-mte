## Definition:

- A random variable X is said to follow a Poisson distribution if it assumes only non-negative values and its probability mass function is given by
- $$P(X = x) = \begin{cases} \frac{e^{-\lambda}\lambda^x}{x!} ; & x = 0,1,2,\ldots\ldots ; \lambda > 0 \\ 0, & \text{otherwise} \end{cases}$$
- Here $\lambda$ is known as the parameter of the distribution.
- We shall use the notation $X \sim P (\lambda)$ to denote that X is a Poisson variate with parameter $\lambda$.
