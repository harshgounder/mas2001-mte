...
## Structural fix: canonical vs derived ledger

The sweep first wrote back onto `reports/evidence/question-instance-ledger.csv`, which broke
the repo's own invariant: that file is GENERATED by `scripts/build_question_instance_ledger.py`
and `check_ledger` rejects any hand-edited copy ("stale generated content"). A ledger that
passes `--check` is the contract.

Fix: the sweep now reads the canonical ledger and writes
`reports/evidence/question-instance-ledger-enriched.csv`. The canonical file stays byte-for-byte
reproducible from its builder (verified CLEAN), and the enrichment lives in its own derived
file. Chain: builder -> canonical ledger -> sweep -> enriched ledger.

## Wave B: first concept MCQ found

`mte-M25-Q2` ("In a Poisson Distribution, if mean (m) = e, then P(x) is given by") is a
verbatim item on Examveda (examveda.com/in-a-poisson-distribution-if-mean-m-e-then-px-is-given-by-268365)
and on a Scribd MCQ set. The other three concept MCQs (M24-A1 Poisson Y=2X, M24-A3 Chebyshev
false-statement, M25-Q3 sufficiency) returned only generic theory pages on the first pass and
need tighter exact-phrase queries.
