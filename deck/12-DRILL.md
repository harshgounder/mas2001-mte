...
Solve, then check. Each item names its distribution and slot. Answers are exact where the
source gives an exact value.

## The drill as a progress ladder

```
   RUNG 1  fundamentals ────── D1-D3     probability, counting, complements
   RUNG 2  discrete dists ──── D4-D17    binomial, Poisson
   RUNG 3  continuous dists ── D18-D31   uniform, normal, exponential
   RUNG 4  Chebyshev ───────── D32-D36   bound, complement, inverse
   RUNG 5  sampling/CLT ────── D37-D41   SE, z, the CLT table
   RUNG 6  estimation ──────── D42-D46   compare, point, interval
   RUNG 7  siblings ────────── D47-D50   the mutation surface

        ┌─────┐
        │ R7  │  siblings (the risk surface)
        ├─────┤
        │ R6  │  estimation
        ├─────┤
        │ R5  │  sampling / CLT
        ...
        │ R4  │  Chebyshev
        ├─────┤
        │ R3  │  continuous distributions
        ├─────┤
        │ R2  │  discrete distributions
        ├─────┤
        │ R1  │  fundamentals
        └─────┘
   climb in order. a rung is "done" when you can do it without notes.
```
...
```
  D11  P(X=1)=0.2 P(X=2). Find lambda. ans lambda = 10
...
```
  ...
  D35  mu=10, var=4, P(5<X<15) and P(|X-10|>=3).  ans 21/25=0.84, and 4/9=0.4444
...
```
   ┌────────────────────────────────────────────────────────────┐
   │ if you can do D4-D46 without notes:                        │
   │    Section B and C of the paper are MECHANICAL             │
   │ if D47-D50 are also fine:                                  │
   │    you are covered on the MUTATION SURFACE                 │
   │ if anything above is shaky:                                │
   │    go back to 04-METHODS.md for that SLOT before more drills│
   └────────────────────────────────────────────────────────────┘
```
